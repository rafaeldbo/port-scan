...
def identify_os_bunner(bunner:str) -> str:
    if bunner is None:
        return None
    if "windows" in bunner or 'microsoft' in bunner:
        return "Windows"
    elif "linux" in bunner:
        return "Linux"
    elif "debian" in bunner:
        return "Debian"
    elif "ubuntu" in bunner:
        return "Ubuntu"
    elif "centos" in bunner:
        return "CentOS"
    elif 'macos' in bunner:
        return "MacOS"
...
def main():
    ...
    group.add_argument("-k",  "--wellknowports",    action="store_true", help="Realiza um scan nas Well Know Ports registradas")
    parser.add_argument(      "--open",             action="store_true", help="Exibe apenas as ports abertas")
    parser.add_argument(      "--os",               action="store_true", help="Tenta identificar o sistema operacional")
    ...
    parser.add_argument("-v", "--validation-ports", action="store",      help=f"Especifique portas, separando por virgula, para serem utilizadas na validação dos hosts da rede (padrão: {DEFAULT_VALIDATION_PORT}) EX.: 22,80,445", default=str(DEFAULT_VALIDATION_PORT))
    ...
    for host in host_list:
        ...
        if args.os:
            ...
            for port in open_ports:
                ...
                os = identify_os_bunner(banner)
                if os is not None:
                    bunner_list.add(os)
            if len(bunner_list) > 0:
                os = " | ".join(bunner_list)
                Console.print(f"Sistema Operacional do host identificado como: {os}", "green")
            else:
